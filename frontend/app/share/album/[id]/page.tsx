  ...
  return (
    <PlaybackProvider>
      <div className="min-h-screen bg-cafe-50 flex flex-col">
        <main className="flex-1 overflow-y-auto px-4 py-8 pb-12">
          <SharedAlbumContent params={params} />
        </main>
        <Player />
      </div>
    </PlaybackProvider>
...
function SharedAlbumContent({ params }: PageProps) {
  const { id } = use(params);
  ...
  function toggleSong(song: Song) {
    if (!song.audio_file_path) {
      setPlaybackError("This song is not ready to play yet.");
      return;
    }
    ...
    const queue = albumData?.songs.filter((item) => item.audio_file_path) || [song];
    playTrack(song, queue);
  }

  if (isLoading) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] text-cafe-700">
    ...
    <div className="max-w-3xl mx-auto">
      ...
      <div className="space-y-3">
        {songs.length === 0 ? (
          ...
          songs.map((song) => {
            const active = currentTrack?.song_id === song.song_id;
            return (
              <div
                key={song.song_id}
